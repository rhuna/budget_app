...
void runApp(Storage& storage) {
    ...
    if (!glfwInit()) return;
    GLFWwindow* window = glfwCreateWindow(1540, 980, "BudgetPilot Monthly Planner v11", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::BeginTabBar("PlannerTabs")) {
            ...
            if (ImGui::BeginTabItem("Calendar Overview")) {
                ImGui::TextWrapped("Calendar overview shows the whole month at once with daily inflow/outflow and event bullets.");
                ImGui::Separator();
                ...
                for (const auto& cell : summary.calendar) { totalIn += cell.inflow; totalOut += cell.outflow; }
                moneyText("Calendar total inflow", totalIn);
                moneyText("Calendar total outflow", totalOut);
                moneyText("Calendar net", totalIn - totalOut);
                ImGui::EndTabItem();
            }
            ...
            if (ImGui::BeginTabItem("Debt Payoff")) {
                ImGui::TextWrapped("Debt payoff strategy compares which account should receive extra money first.");
                ImGui::Separator();
            ...
            if (ImGui::BeginTabItem("Paycheck Templates")) {
                ImGui::TextWrapped("Templates let you apply repeatable paycheck allocations without retyping every month.");
                ImGui::Separator();
