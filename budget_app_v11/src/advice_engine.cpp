...
DebtPlanResult buildDebtPlan(const BudgetData& data, double monthlyLeftover) {
    ...
    if (targetMonthly > 0.0) result.estimatedMonthsToPrimaryPayoff = static_cast<int>(std::ceil(targetBalance / targetMonthly));
    return result;
}

double monthlyIncomeBase(const BudgetData& data) {
    double total = 0.0;
    for (const auto& income : data.incomes) {
        if (income.frequency == Frequency::BiWeekly) total += income.amount * 2.0;
        else if (income.frequency == Frequency::Monthly) total += income.amount;
        else total += monthlyFrom(income.amount, income.frequency);
    }
    return total;
}

double monthlyBillsBase(const BudgetData& data) {
    double total = 0.0;
    for (const auto& bill : data.bills) total += monthlyFrom(bill.amount, bill.frequency);
    return total;
}

double monthlyVariableBase(const BudgetData& data) {
    double total = 0.0;
    for (const auto& item : data.variableExpenses) total += item.monthlyPlan;
    return total;
}

double monthlyGoalsBase(const BudgetData& data) {
    double total = 0.0;
    for (const auto& goal : data.goals) total += goal.monthlyContribution;
    return total;
}

double monthlyOneTimeBase(const BudgetData& data) {
    double total = 0.0;
    for (const auto& item : data.oneTimeExpenses) if (item.active) total += item.amount;
    return total;
}

ScenarioResult runScenario(const BudgetData& data, const Scenario& scenario) {
    ScenarioResult r{};
    r.scenarioName = scenario.name;

    const double baseIncome = monthlyIncomeBase(data);
    const double baseBills = monthlyBillsBase(data);
    const double baseVariable = monthlyVariableBase(data);
    const double baseGoals = monthlyGoalsBase(data);
    const double baseOneTime = monthlyOneTimeBase(data);

    r.monthlyIncome = baseIncome * scenario.incomeMultiplier + scenario.extraMonthlyIncome;
    r.monthlyOutflow =
        (baseBills * scenario.billMultiplier) +
        (baseVariable * scenario.variableMultiplier) +
        baseGoals + baseOneTime + data.monthlyTaxReserve +
        scenario.extraMonthlyBills + scenario.oneTimeShock;
    r.monthlyLeftover = r.monthlyIncome - r.monthlyOutflow;

    double checking = data.checkingBalance;
    double emergency = data.emergencyFundBalance;
    double debt = data.debtBalance;
    const int months = std::max(1, data.settings.forecastMonths);

    for (int i = 0; i < months; ++i) {
        checking += r.monthlyLeftover;
        if (checking < 0.0 && r.firstNegativeMonth == -1) r.firstNegativeMonth = i + 1;
        if (r.monthlyLeftover > 0.0) {
            emergency += r.monthlyLeftover * 0.35;
            debt = std::max(0.0, debt - r.monthlyLeftover * 0.65);
        } else {
            emergency = std::max(0.0, emergency + r.monthlyLeftover * 0.20);
            debt += std::abs(r.monthlyLeftover) * 0.30;
        }
    }

    r.endingChecking = checking;
    r.endingEmergencyFund = emergency;
    r.endingDebt = debt;
    return r;
}
...
BudgetSummary buildSummary(const BudgetData& data) {
    ...
    s.monthlyIncome = monthlyIncomeBase(data);
    ...
    for (const auto& item : data.variableExpenses) {
        ...
        s.chartCategoryActuals.push_back(item.actual);
    }
    ...
    for (const auto& h : data.history) {
        s.chartHistoryLeftover.push_back(h.leftover);
        s.chartHistoryDebt.push_back(h.debtBalance);
        s.chartNetWorthTrend.push_back(h.emergencyFund - h.debtBalance);
    }
    ...
    double emergency = data.emergencyFundBalance;
    double debt = data.debtBalance;
    const int forecastMonths = std::max(1, data.settings.forecastMonths);
    for (int i = 0; i < forecastMonths; ++i) {
        ForecastMonth fm;
        fm.label = "M+" + std::to_string(i + 1);
        fm.projectedIncome = s.monthlyIncome;
        fm.projectedOutflow = s.monthlyPlannedOutflow;
        fm.projectedLeftover = fm.projectedIncome - fm.projectedOutflow;
        checking += fm.projectedLeftover;
        if (fm.projectedLeftover > 0.0) {
            emergency += fm.projectedLeftover * 0.35;
            debt = std::max(0.0, debt - fm.projectedLeftover * 0.65);
        } else {
            emergency = std::max(0.0, emergency + fm.projectedLeftover * 0.20);
            debt += std::abs(fm.projectedLeftover) * 0.30;
        }
        fm.projectedChecking = checking;
        fm.projectedEmergencyFund = emergency;
        fm.projectedDebt = debt;
        s.chartForecastChecking.push_back(checking);
        s.chartForecastDebt.push_back(debt);
        s.forecast.push_back(fm);
    }

    // Scenarios
    for (const auto& scenario : data.scenarios) s.scenarioResults.push_back(runScenario(data, scenario));

    // Daily balance engine
    double dailyBalance = data.checkingBalance;
    for (int day = 1; day <= monthDays; ++day) {
        DailyBalancePoint p;
        p.day = day;
        p.balance = dailyBalance;
        p.eventLabel = "No major event";

        for (const auto& income : data.incomes) {
            if (income.frequency == Frequency::BiWeekly && (income.payDay1 == day || income.payDay2 == day)) {
                dailyBalance += income.amount;
                p.eventLabel = "Income: " + income.name;
            } else if (income.frequency == Frequency::Monthly && income.payDay1 == day) {
                dailyBalance += income.amount;
                p.eventLabel = "Income: " + income.name;
            }
        }
        ...
        for (const auto& bill : data.bills) {
            if (bill.frequency == Frequency::Monthly && bill.dueDay == day) {
                dailyBalance -= bill.amount;
                p.eventLabel = "Bill: " + bill.name;
            }
        }

        for (const auto& tx : data.transactions) {
            if (tx.dateLabel.size() >= 10) {
                try {
                    const int txDay = std::stoi(tx.dateLabel.substr(8, 2));
                    if (txDay == day) {
                        if (tx.type == TransactionType::Expense) dailyBalance -= tx.amount;
                        else dailyBalance += tx.amount;
                        p.eventLabel = "Ledger: " + tx.note;
                    }
                } catch (...) {}
            }
        }
    ...
    for (const auto& item : data.variableExpenses) {
        ...
        if (overspend > 0.0) {
            s.optimizations.push_back({"Reduce " + item.name + " to plan level", overspend});
        }
    }
    if (s.monthlyPlannedLeftover < 0.0) {
        s.optimizations.push_back({"Cut nonessential spending categories first", std::abs(s.monthlyPlannedLeftover)});
    }
    if (!s.debtPlan.recommendedTarget.empty() && s.monthlyPlannedLeftover > 0.0) {
        s.optimizations.push_back({"Send leftover to " + s.debtPlan.recommendedTarget, s.monthlyPlannedLeftover});
    }

    // Alerts
    bool foundNegativeDay = false;
    for (const auto& p : s.dailyBalance) {
        if (p.balance < 0.0) {
            s.alerts.push_back({"HIGH", "Checking goes negative on day " + std::to_string(p.day) + "."});
            foundNegativeDay = true;
            break;
        }
    }
    if (!foundNegativeDay && s.monthlyPlannedLeftover < 100.0) {
        s.alerts.push_back({"MEDIUM", "Monthly leftover is very thin. One surprise expense could break cashflow."});
    }
    if (!s.forecast.empty() && s.forecast.back().projectedChecking < 0.0) {
        s.alerts.push_back({"HIGH", "Forecast ends with negative checking balance."});
    }
    if (s.emergencyMonthsCovered < 1.0) {
        s.alerts.push_back({"MEDIUM", "Emergency fund is below one month of essentials."});
    }
...
std::vector<std::string> buildAdvice(const BudgetData& data, const BudgetSummary& summary) {
    ...
    if (!data.history.empty()) advice.emplace_back("Monthly closeout history is active. Compare leftover cashflow and debt over time.");
    if (!summary.forecast.empty() && summary.forecast.back().projectedChecking < 0.0) advice.emplace_back("Forecast alert: projected checking goes negative before the end of the forecast window.");
    if (!summary.scenarioResults.empty()) advice.emplace_back("Scenario planner is active. Compare cases side by side before making major money moves.");
    if (!summary.alerts.empty()) advice.emplace_back("Core engine alerts are active. Review overdraft and low-buffer warnings.");
    advice.emplace_back("This build adds bank import, automation rules, and a decision simulator on top of v12-era full functionality and later upgrades.");
    return advice;
}
