...
void applyAutoCategorization(BudgetData& data) {
    for (auto& tx : data.transactions) {
        if (tx.type != TransactionType::Expense) continue;
        const std::string search = lower(tx.note + " " + tx.category);
        for (const auto& rule : data.autoCategoryRules) {
            if (search.find(lower(rule.merchantContains)) != std::string::npos) {
...
bool Storage::load(BudgetData& data, std::string& error) const {
    ...
    while (std::getline(in, line)) {
        ...
        } else if (parts[0] == "SCENARIO" && parts.size() >= 9) {
            data.scenarios.push_back({parts[1], toDouble(parts[2]), toDouble(parts[3]), toDouble(parts[4]), toDouble(parts[5]), toDouble(parts[6]), toDouble(parts[7]), toBool(parts[8], false)});
        } else if (parts[0] == "RULE" && parts.size() >= 3) {
            data.autoCategoryRules.push_back({parts[1], parts[2]});
        } else if (parts[0] == "ASSET" && parts.size() >= 4) {
            data.accountAssets.push_back({parts[1], parts[2], toDouble(parts[3])});
        }
    }
...
bool Storage::save(const BudgetData& data, std::string& error) const {
    ...
    for (const auto& x : data.autoCategoryRules) out << "RULE|" << x.merchantContains << "|" << x.category << "\n";
    for (const auto& x : data.accountAssets) out << "ASSET|" << x.name << "|" << x.type << "|" << x.balance << "\n";
    return true;
}
...
BudgetData makeSampleData() {
    ...
    d.transactions = {{"2026-04-02", "Misc", "Walmart", 92.15, TransactionType::Expense}, {"2026-04-03", "Misc", "Shell", 34.20, TransactionType::Expense}, {"2026-04-05", "Paycheck", "Side work", 250.00, TransactionType::Income}, {"2026-04-06", "Misc", "DoorDash", 22.50, TransactionType::Expense}};
    ...
    d.autoCategoryRules = {{"walmart", "Groceries"}, {"shell", "Gas"}, {"doordash", "Eating Out"}, {"spotify", "Subscriptions"}};
    d.accountAssets = {{"Checking", "Cash", 900.0}, {"Emergency Savings", "Savings", 1200.0}, {"Brokerage", "Investments", 2500.0}};
