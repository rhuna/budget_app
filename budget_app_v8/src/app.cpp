...
void runApp(Storage& storage) {
    ...
    if (!glfwInit()) return;
    GLFWwindow* window = glfwCreateWindow(1540, 990, "BudgetPilot Monthly Planner v13", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::SetNextWindowSize(ImVec2(1520, 970), ImGuiCond_Once);
        ...
        if (ImGui::BeginTabBar("PlannerTabs")) {
            if (ImGui::BeginTabItem("Dashboard + Charts")) {
                ...
                drawMiniBars("##planactual", summary.chartPlanVsActual, 110.0f);
                ImGui::TextUnformatted("Paycheck leftover trend");
                drawMiniBars("##payleftover", summary.chartPaycheckLeftovers, 110.0f);
            ...
            if (ImGui::BeginTabItem("History + Insights")) {
                ...
                drawMiniBars("##historyleftover", summary.chartHistoryLeftover, 110.0f);
                ImGui::TextUnformatted("Debt balance history");
                drawMiniBars("##historydebt", summary.chartHistoryDebt, 110.0f);
                ImGui::TextUnformatted("Current category actuals");
                drawMiniBars("##categoryactuals", summary.chartCategoryActuals, 110.0f);
            ...
            if (ImGui::BeginTabItem("Scenario Planner")) {
                ImGui::TextWrapped("Create what-if scenarios and compare outcomes over the forecast window.");
                static char scenarioName[128] = "";
                static float incomeMul = 1.0f;
                static float billMul = 1.0f;
                static float varMul = 1.0f;
                static float extraIncome = 0.0f;
                static float extraBills = 0.0f;
                static float shock = 0.0f;

                ImGui::Separator();
                ImGui::InputText("Scenario name", scenarioName, IM_ARRAYSIZE(scenarioName));
                ImGui::InputFloat("Income multiplier", &incomeMul, 0.01f, 0.05f, "%.2f");
                ImGui::InputFloat("Bill multiplier", &billMul, 0.01f, 0.05f, "%.2f");
                ImGui::InputFloat("Variable multiplier", &varMul, 0.01f, 0.05f, "%.2f");
                ImGui::InputFloat("Extra monthly income", &extraIncome, 10.0f, 50.0f, "%.2f");
                ImGui::InputFloat("Extra monthly bills", &extraBills, 10.0f, 50.0f, "%.2f");
                ImGui::InputFloat("One-time shock", &shock, 10.0f, 100.0f, "%.2f");
                if (ImGui::Button("Add scenario") && scenarioName[0] != '\\0') {
                    data.scenarios.push_back({scenarioName, incomeMul, billMul, varMul, extraIncome, extraBills, shock, false});
                    scenarioName[0] = '\\0';
                    incomeMul = billMul = varMul = 1.0f;
                    extraIncome = extraBills = shock = 0.0f;
                }

                ImGui::Separator();
                for (std::size_t i = 0; i < data.scenarios.size(); ++i) {
                    auto& s = data.scenarios[i];
                    ImGui::PushID(static_cast<int>(i) + 14000);
                    if (ImGui::TreeNode((s.name + "##scenario").c_str())) {
                        ...
                        if (ImGui::Button("Delete scenario")) {
                            data.scenarios.erase(data.scenarios.begin() + static_cast<long long>(i));
                            ImGui::TreePop();
                            ImGui::PopID();
                            break;
                        }
                ...
                ImGui::Separator();
                ImGui::TextUnformatted("Scenario results");
            ...
            if (ImGui::BeginTabItem("Multi-Month Forecast")) {
                ImGui::TextWrapped("Forecast shows projected checking, emergency fund, and debt over the next months based on the current plan.");
                ImGui::Separator();
                ImGui::TextUnformatted("Projected checking");
                drawMiniBars("##forecastchecking", summary.chartForecastChecking, 110.0f);
                ImGui::TextUnformatted("Projected debt");
                drawMiniBars("##forecastdebt", summary.chartForecastDebt, 110.0f);

                ImGui::Separator();
                for (const auto& month : summary.forecast) {
                    if (ImGui::TreeNode(month.label.c_str())) {
                        moneyText("Projected income", month.projectedIncome);
                        moneyText("Projected outflow", month.projectedOutflow);
                        moneyText("Projected leftover", month.projectedLeftover);
                        moneyText("Projected checking", month.projectedChecking);
                        moneyText("Projected emergency fund", month.projectedEmergencyFund);
                        moneyText("Projected debt", month.projectedDebt);
                        ImGui::TreePop();
                    }
                }
                ImGui::EndTabItem();
