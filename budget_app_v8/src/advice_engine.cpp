...
BudgetSummary buildSummary(const BudgetData& data) {
    ...
    for (const auto& scenario : data.scenarios) {
        s.scenarioResults.push_back(runScenario(data, scenario));
    }
...
std::vector<std::string> buildAdvice(const BudgetData& data, const BudgetSummary& summary) {
    ...
    advice.emplace_back("This v13 upgrade keeps v12 and adds scenario planning with multi-month forecasting.");
    return advice;
}
