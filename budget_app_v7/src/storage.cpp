...
bool Storage::save(const BudgetData& data, std::string& error) const {
    ...
    out << "META|" << data.checkingBalance << "|" << data.emergencyFundBalance << "|" << data.debtBalance << "|" << data.savingsGoal << "|" << data.monthlyTaxReserve << "|" << data.settings.month << "|" << data.settings.year << "|" << data.settings.uiScale << "|" << data.settings.paycheckCount << "|" << fromDebtStrategy(data.settings.debtStrategy) << "\n";
