...
std::vector<std::string> buildAdvice(const BudgetData& data, const BudgetSummary& summary) {
    ...
    if (!data.history.empty()) advice.emplace_back("Monthly closeout history is now active. Use it to compare leftover cashflow and debt over time.");
    advice.emplace_back("This v12 upgrade keeps everything from v11 and adds monthly closeout history plus insights charts.");
    return advice;
}
